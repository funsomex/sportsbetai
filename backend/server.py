...
async def get_bet_of_the_day():
    ...
    if THE_ODDS_API_KEY:
        try:
            ...
            if matches is not None and len(matches) > 0:
                value_bets = calculate_real_value_bets(matches)
                if value_bets:
                    # Get the best value bet (highest value percentage with good confidence)
    ...
    if not best_bet:
        return {"bet": None, "message": "No hay apuestas disponibles hoy"}
...
async def generate_auto_parlay(request: AutoParlayRequest):
    ...
    """
    try:
        # Get real matches first, fallback to mock
        matches = await get_all_real_matches()
        source = "real"
        ...
        if not matches:
            ...
            matches = generate_mock_matches(count=50)
            source = "demo"
            # For mock data, ensure matches are in the future for the requested date filter
            now = datetime.now(timezone.utc)
            for m in matches:
                if request.date_filter == "today":
                    m["start_time"] = (now + timedelta(hours=random.randint(1, 10))).isoformat()
                elif request.date_filter == "tomorrow":
                    tomorrow = now + timedelta(days=1)
                    m["start_time"] = (tomorrow.replace(hour=random.randint(10, 22))).isoformat()
                elif request.date_filter == "week":
                    m["start_time"] = (now + timedelta(days=random.randint(0, 6), hours=random.randint(10, 22))).isoformat()
